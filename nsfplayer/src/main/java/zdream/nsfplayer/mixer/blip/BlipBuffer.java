package zdream.nsfplayer.mixer.blip;

import java.util.Arrays;

import zdream.nsfplayer.core.NsfPlayerException;

import static zdream.nsfplayer.mixer.blip.BufferContext.blip_max_length;
import static zdream.nsfplayer.mixer.blip.BufferContext.blip_sample_bits;
import static zdream.nsfplayer.mixer.blip.BufferContext.blip_widest_impulse_;
import static zdream.nsfplayer.mixer.blip.BufferContext.buffer_extra;


/**
 * <p>Source file Blip_Buffer 0.4.0
 * <p>Band-limited sound synthesis and buffering
 *
 * @author Zdream
 */
public class BlipBuffer {

    /**
     * <p>Set output sample rate and buffer length in milliseconds (1/1000 sec, defaults
     * to 1/4 second), then clear buffer.<br>
     * If there isn't enough memory, throw error without affecting current buffer setup.
     *
     * @param new_rate The output sampling rate, how many samples per second
     * @param msec     The buffer length, in milliseconds, is called by default with a value of 1000 / 4 = 250
     * @throws RuntimeException When memory is insufficient
     */
    public void setSampleRate(int new_rate, int msec) throws RuntimeException {
        int new_size = 65535 - buffer_extra - 64;
        if (msec != blip_max_length) {
            int s = (new_rate * (msec + 1) + 999) / 1000;
            if (s < new_size)
                new_size = s;
            else
                throw new RuntimeException("The requested buffer length has exceeded the maximum limit");
        }

        if (buffer_size_ != new_size) {
            buffer_ = new long[new_size + buffer_extra];
        }

        buffer_size_ = new_size;

        // update things based on the sample rate
        sample_rate_ = new_rate;
        length_ = new_size * 1000 / new_rate - 1;

        if (msec != 0)
            assert (length_ == msec); // ensure length is same as that passed in
        if (clock_rate_ != 0)
            clockRate(clock_rate_);
        bassFreq(bass_freq_);

        clear(true);

        // success and return
    }

    /**
     * <p>Set number of source time units per second
     *
     * @param cps Global input sampling rate, in clocks per second for NSF tracks
     */
    public void clockRate(int cps) {
        factor_ = clockRateFactor(clock_rate_ = cps);
    }

    /**
     * <p>End current time frame of specified duration and make its samples available
     * (along with any still-unread samples) for reading with read_samples(). Begins
     * a new time frame at the end of the current frame.
     *
     * @param time The specified duration, in clock cycles
     */
    public void endFrame(int time) {
        offset_ += time * factor_;
        int avail = samplesAvail();
        assert (avail <= buffer_size_); // time outside buffer length
    }

    /**
     * <p>Read at most {@code max_samples} out of buffer into {@code dest}, removing them from
     * the buffer. Returns number of samples actually read and removed. If stereo is
     * true, increments 'dest' one extra time after writing each sample, to allow
     * easy interleving of two channels into a stereo output buffer.
     * <p>Also note that because the 16-bit sample data is written into a byte array with 8 bits per unit,
     * one sample data occupies two bytes.
     *
     * @param dest        Already converted to byte array
     * @param offset      The default value is 0, indicating where the dest array starts writing data.<br>
     *                    When mono (stereo = false), usually 0<br>
     *                    When stereo (stereo = true), channel 1 is usually 0 and channel 2 is 1
     * @param max_samples This is a 16-bit sample, so it should be less than or equal to dest.length
     * @param stereo      Default is false. If true, it means stereo
     * @return
     */
    public int readSamples(short[] dest, int offset, int max_samples, boolean stereo) {
        int count = Math.min(samplesAvail(), max_samples);

        if (count != 0) {
            final int sample_shift = blip_sample_bits - 16;
            long accum = reader_accum;
            int ptr = 0;
            int outptr = offset;

            if (!stereo) {
                for (int n = count; (--n) >= 0; ) {
                    long s = (accum >> sample_shift);
                    accum -= accum >> bass_shift;
                    accum += buffer_[ptr++];

                    short out = (short) s;

                    // clamp sample
                    if (out != s)
                        out = (short) (0x7FFF - (s >> 24));

                    dest[outptr++] = out;
                    if (out != 0) {
                        out += 0;
                    }
                }
            } else {
                for (int n = count; (--n) >= 0; ) {
                    long s = (accum >> sample_shift);
                    accum -= accum >> bass_shift;
                    accum += buffer_[ptr++];

                    short out = (short) s;

                    // clamp sample
                    if (out != s)
                        out = (short) (0x7FFF - (s >> 24));

                    dest[outptr++] = out;

                    outptr += 1;
                }
            }

            reader_accum = accum;
            removeSamples(count);
        }
        return count;
    }

// Additional optional features

    /**
     * <p>Sampling rate
     * <p>Current output sample rate, const function
     *
     * @return
     */
    public final int sampleRate() {
        return sample_rate_;
    }

    public void sampleRate(int r) {
        setSampleRate(r, 250);
    }

    public void sampleRate(int r, int msec) {
        setSampleRate(r, msec);
    }

    /**
     * <p>Buffer The length of the time in milliseconds
     * <p>Length of buffer, in milliseconds
     *
     * @return
     */
    public final int length() {
        return length_;
    }

    /**
     * <p>Number of source time units per second
     *
     * @return
     */
    public final int clockRate() {
        return clock_rate_;
    }

    /**
     * <p>Set frequency high-pass filter frequency, where higher values reduce bass more
     *
     * @param freq
     */
    public void bassFreq(int freq) {
        bass_freq_ = freq;
        int shift = 31;
        if (freq > 0) {
            shift = 13;
            long f = ((long) freq << 16) / sample_rate_;
            while ((f >>= 1) > 0 && (--shift > 0)) {
            }
        }
        bass_shift = shift;
    }

    /**
     * <p>Number of samples delay from synthesis to samples read out
     *
     * @return
     */
    public int output_latency() {
        return blip_widest_impulse_ / 2;
    }

    /**
     * <p>Remove all available samples and clear buffer to silence.<br>
     * If {@code entire} is false, just clears out any samples waiting rather than the entire buffer.
     *
     * @param entire entire_buffer, Whether to clear the entire buffer data. Default true
     */
    public void clear(boolean entire) {
        offset_ = 0;
        reader_accum = 0;
        if (buffer_ != null) {
            int count = (entire ? buffer_size_ : samplesAvail());
            Arrays.fill(buffer_, 0, count + buffer_extra, 0);
        }
    }

    /**
     * <p>Number of samples available for reading with read_samples()
     *
     * @return
     */
    public final int samplesAvail() {
        return offset_ >> 16;
    }

    /**
     * <p>Remove 'count' samples from those waiting to be read
     *
     * @param count
     */
    public void removeSamples(int count) {
        removeSilence(count);

        // copy remaining samples to beginning and clear old samples
        int remain = samplesAvail() + buffer_extra;
        long[] buffer2 = new long[buffer_.length];
        System.arraycopy(buffer_, count, buffer2, 0, remain);

        buffer_ = buffer2;
    }

    // Experimental features

    /**
     * <p>Number of raw samples that can be mixed within frame of specified duration.
     *
     * @param duration
     * @return
     */
    public final int countSamples(int duration) {
        int last_sample = resampledTime(duration) >> 16;
        int first_sample = offset_ >> 16;
        return last_sample - first_sample;
    }

    /**
     * <p>Mix 'count' samples from 'buf' into buffer.
     */
    public void mixSamples(short[] buf, int offset, int count) {
        int outptr = (offset_ >> 16) + blip_widest_impulse_ / 2;
        int inptr = offset;

        final int sample_shift = blip_sample_bits - 16;
        long prev = 0;
        while (count-- > 0) {
            long s = buf[inptr++] << sample_shift;
            buffer_[outptr] += s - prev;
            prev = s;
            ++outptr;
        }
        buffer_[outptr] -= prev;
    }

    /**
     * <p>Count number of clocks needed until 'count' samples will be available.
     * If buffer can't even hold 'count' samples, returns number of clocks until
     * buffer becomes full.
     *
     * @param count
     * @return
     */
    public final int countClocks(int count) {
        if (count > buffer_size_)
            count = buffer_size_;
        int time = count << 16;
        return ((time - offset_ + factor_ - 1) / factor_);
    }

    // not documented yet
    public void removeSilence(int count) {
        assert (count <= samplesAvail()); // tried to remove more samples than available
        offset_ -= count << 16;
    }

    public final int resampledDuration(int t) {
        return t * factor_;
    }

    public final int resampledTime(int t) {
        return t * factor_ + offset_;
    }

    int clockRateFactor(int clock_rate) {
        double ratio = (double) sample_rate_ / clock_rate;
        int factor = (int) Math.floor(ratio * (1L << 16) + 0.5);
        if (factor <= 0 && sample_rate_ != 0) {
            throw new NsfPlayerException(
                    String.format("The ratio of input sampling rate %d to output sampling rate %d is too large", clock_rate, sample_rate_));
        }
        return factor;
    }

    public BlipBuffer() {
        factor_ = Integer.MAX_VALUE;
        bass_freq_ = 16;
    }

    /**
     * I guess, milliseconds * factor_ = number of samples
     */
    public int factor_;
    public int offset_;

    /**
     * This is the location where the sampled data is stored, which is the calculated sampled data
     */
    long[] buffer_;
    int buffer_size_;

    /**
     * According to my guess, each calculated sample data value is different from the actual sample
     * data value played. The data played at the next sample point is actually determined by the
     * calculated value of the previous sample point and the calculated value of the next sample point.
     * Therefore, it is necessary to store the calculated value of the previous sampling point
     */
    long reader_accum; // Determined to be long
    int bass_shift;
    int sample_rate_;

    /**
     * The number of clocks per second, i.e. the global input sampling rate
     */
    int clock_rate_;
    private int bass_freq_;
    int length_;
}
