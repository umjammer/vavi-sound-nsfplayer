...
import zdream.utils.common.BitStream;


public class Layer3 extends AbstractLayer {

    private int ngr;        // Number of granules
    private int nch;        // Number of channels
    private MpegFrameHead header;
    private BitStream bsSI;    // Read frame side information (Side Information) bit stream
    private BitStreamMainData maindataStream;

    /**
     * The starting position of the main data after removing the frame edge information, pointing to the bsSI buffer
     */
...
//	private int nadc;
    ...
    /*
     * A lot of data here is obtained
     */
    ...
    @Override
    public void ready(MpegFrameHead head) {
        if (isNew) {
            init(head);
            isNew = false;
            return;
    ...
    protected void init(MpegFrameHead head) {
        ...
        // Used to look up the table to find v^(4/3), where v is a (positive) value decoded by Huffman,
        // and the value range is 0..8191
        floatPowIS = new float[8207];
        for (i = 0; i < 8207; i++)
            floatPowIS[i] = (float) Math.pow(i, 4.0 / 3.0);

        // Used to look up the table to find 2^(-0.25 * i)
        // According to the formula, the maximum value of i in short blocks is: 210 - 0 + 8 * 7 + 4 * 15 + 2 = 328
        // Minimum value of i for long or short blocks: 210 - 255 + 0 + 0 + 0 = -45
        // When using the table lookup method, the subscript range is 0..328+45.
        floatPow2 = new float[328 + 46];
        for (i = 0; i < 374; i++)
            floatPow2[i] = (float) Math.pow(2.0, -0.25 * (i - 45));

        //
        // Different variables are used for different features of the file to be decoded. Initialization:
        //
        int sfreq = header.getSamplingFrequency();
        sfreq += isMPEG1 ? 0 : (header.isMPEG2() ? 3 : 6);
    ...
    public byte[] decode(byte[] bs, int offset, int length) {
        ...
        /*
         * part1 : Frame edge information
         */
        int i = getSideInfo(bs, offset); // The number of bytes occupied by the frame edge information
        if (i < 0)
            return null; // Skip this frame's main data
        off = i;

        /*
         * part2_3: Gain factor + Huffman bit
         * length: ((part2_3_bits + 7) >> 3) bytes
         */
        int endPos = maindataStream.getSize();

        // Discard the padding bits of the previous frame
        int discard = endPos - maindataStream.getBytePos() - main_data_begin;
        maindataStream.skipBytes(discard);

        // Main data is added to the bitstream buffer
        maindataStream.append(bs, off, length);
        off += length;
        ...
        /*
         * You can call maindataStream.skipBits(part2_3_bits & 7) here to discard the padding bits.
         * A better way is to process it before decoding the next frame of main data.
         *  If the bit stream is wrong, it can be corrected by the way.
         */

        // Single-thread serial decoding of music data to complete the remaining 4 steps
    ...
    /**
     * Process frame edge information and modify the data in infoCh0 and infoCh1
     *
     * @param b
     * @param off
     * @return After processing the frame edge information, the index pointing to the main audio data
     */
    ...
    private void getScaleFactors(int gr, int ch) {
        int scf;
        ChannelInformation ci = (ch == 0) ? infoCh0[gr] : infoCh1[gr];
    ...
    //3.
    //>>>>HUFFMAN BITS=========================================================
    private int[] region;    // A code table is used to decode the area of the main data (up to 3 segments)
    private int[] hv;        // [32 * 18 + 4], temporarily store the solved Huffman value
    ...
    private void huffBits(int gr, int ch) {
        ...
        if (ci.window_switching_flag == 1) {
            if (header.isMPEG1() || (header.isMPEG2() && ci.block_type == 2)) {
                region1Start = 36;
                region2Start = 576;
            } else {
                if (header.isMPEG25()) {
        ...
        if (bv > 574)
            bv = 574; // Wrong big_value is set to 0?
        if (region1Start < bv) {
        ...
        ci.nonzero = maindataStream.decodeHuff(region, ci.table_select, ci.count1table_select, hv);  // Huffman decoding
    }
    //<<<<HUFFMAN BITS=========================================================
    ...
    private int[] widthLong;    // [22] Gain factor band for long blocks (number of frequency lines inverse quantized with one gain factor)
    private int[] widthShort;    // [13] Gain factor band for short blocks
    ...
    /**
     * Inverse quantize and reorder short blocks (pure short blocks and short blocks in mixed blocks). Variables assigned during inverse quantization:<br>
     * rzero_bandL -- Number of bands with non-zero Huffman values for long blocks, used for intensity stereo processing<br>
     * rzero_bandS -- Number of bands with non-zero Huffman values for short blocks, used for intensity stereo processing<br>
     * rzero_index -- Number of "subbands" with non-zero Huffman values
     * <p>
     * Layer3 inverse quantization formula ISO/IEC 11172-3, 2.4.3.4
     * <p>
    ...
    private void requantizer(int ch, ChannelInformation ci) {
        int[] l = scalefacLong[ch];
        float[] buf = ci.buf;
        boolean preflag = ci.preflag == 1;
        int shift = 1 + ci.scalefac_scale;
        int maxi = ci.nonzero;
        float requVal;
        int bi = 0, sfb = 0, width, pre, val, hvidx = 0, xri = 0, scf = 0;
        int xriStart = 0; // Used to calculate the subscript after short block reordering
        int pow2i = 255 - ci.global_gain;

        if (header.isMS())
            pow2i += 2; // If the channel mode is ms_stereo, divide by the root of 2
        ...
        if (ci.window_switching_flag == 1 && ci.block_type == 2) {
            ...
            if (ci.mixed_block_flag == 1) {
                /*
                 * Mixing block:
                 * The first 8 frequency bands of the mixed block are long blocks. Each of the first 8 blocks
                 * is inverse quantized with a gain factor, and the sum of the frequency bands of these 8 gain
                 * factors is 36. These 36 frequency lines are inverse quantized using the long block formula.
                 */
                rzeroBandLong = -1;
                for (; sfb < 8; sfb++) {
                ...
                /*
                 * The last 9 frequency bands of the mixing block are short windowed blocks,
                 * and the gain factor bands of the 3 values in the same window of each block are the same.
                 * The frequency subband value corresponding to the gain factor of the last 9 blocks is
                 *  widthShort[3..11]
                 */
                ...
                sfb = 3;
                scf = 9;
                xriStart = 36; // Prepare indexes for short block reordering
            }

            // Short blocks (short blocks in mixed blocks and pure short blocks)
            int[] s = scalefacShort[ch];
        ...
        // Do not inverse quantize the 0 value area and set it to 0.
        for (; hvidx < 576; hvidx++)
            buf[hvidx] = 0;
    }
    //<<<<REQUANTIZATION & REORDER=============================================
    ...
    // The division by root 2 is already done in the requantizer method, and it is no longer done in ms_stereo.
    private void ms_stereo(int gr) {
        ...
        infoCh0[gr].nonzero = infoCh1[gr].nonzero = nz; //...Failure to do so may result in loss of sound detail
    }

    private float[][] is_coef_lsf;
    private float[] is_coef;

    // Decode one band intensity stereo, MPEG-1
    ...
    // Decode one band intensity stereo, MPEG-2
     ...
     *
     * ISO/IEC 11172-3 does not perform intensity stereo processing on long blocks in mixed blocks,
     * but many MP3 decoders do.
     */
    private void intensity_stereo(int gr) {
        ChannelInformation ci = infoCh1[gr]; // Information is stored in the right channel
        int scf, idx, sfb;
        ...
        if (isMPEG1) {    //MPEG-1
            if (ci.block_type == 2) {
                ...
                int w3;
                for (w3 = 0; w3 < 3; w3++) {
                    sfb = rzeroBandShort[w3]; // The minimum mixed block sfb is 3
                    for (; sfb < 12; sfb++) {
    ...
    private void imdct36(float[] preBlck, float[] recv, int off, int block_type) {
        ...
        // Using Byeong Gi Lee's algorithm
    ...
    private float[] preBlckCh0; // [32*18],Left channel FIFO queue
    private float[] preBlckCh1; // [32*18],Right channel FIFO
    ...
    private void hybrid(int ch, ChannelInformation ci) {
        ...
        boolean mixed = (ci.window_switching_flag == 1 && ci.mixed_block_flag == 1);
    ...
    //
    // Implementing polyphase frequency inversion in the decodeAudioDataChannel method
    //
    //<<<<INVERSE QUANTIZE SAMPLES=============================================
    ...
    //
    // jmp123.decoder.Synthesis.synthesisSubBand method
    //
    //<<<<SYNTHESIZE VIA POLYPHASE MDCT========================================
    ...
    /*
     * samplesChx: Temporarily store 32 sample values extracted from the result of sub-band synthesis
     * (hybrid), These 32 sample values are sent to the polyphase synthesis filter as input data.
     */
    private float[] samplesCh0, samplesCh1;

    // One channel anti-aliasing, sub-band synthesis, polyphase frequency inversion
    // and polyphase synthesis filtering
    private void decodeAudioDataChannel(ChannelInformation[] ci, int ch) {
        ...
        for (gr = 0; gr < ngr; gr++) {
            antialias(ch, ci[gr]);    //Anti-aliasing
            hybrid(ch, ci[gr]);        //Sub-band synthesis

            b = ci[gr].buf;
            for (ss = 0; ss < 18; ss += 2) {
                for (i = ss, sb = 0; sb < 32; sb++, i += 18)
                    samples[sb] = b[i];
                synthesisSubBand(samples, ch); //Polyphase Synthesis Filter

                for (i = ss + 1, sb = 0; sb < 32; sb += 2, i += 36) {
                    samples[sb] = b[i];
                    samples[sb + 1] = -b[i + 18]; // Multiphase Frequency Inversion (INVERSE QUANTIZE SAMPLES)
                }
    ...
    /*
     * Information of one channel within one granularity
     */
    private static class ChannelInformation {

        // Read data from the bit stream and initialize 14 variables in the following order, see the getSideInfo method.
        ...
        private int part2_length;    // Scale-factor bits
        private int nonzero;        // The total length of the large value area and the small value area
        private final float[] buf;  // Temporarily store the intermediate results of inverse quantization, anti-aliasing, sub-band synthesis, etc. of a channel within a granularity
