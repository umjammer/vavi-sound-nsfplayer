...
/**
 * Used to read the bit stream and decode the Huffman data
 * when decoding MPEG-1/MPEG-2/MPEG-2.5 Audio Layer Ⅲ main data (main_data).
 *
 * @author jmp123
 */
public class BitStreamMainData extends BitStream {

    // Large value area for Huffman decoding
    private int[] lin;        // lin[0..15]=0
    private short[][] htbv;

    // Used to decode small value areas
    private short[] htc0, htc1;

    /**
     * Create a bitstream BitStreamMainData object. The bitstream buffer size is specified by len,
     * and the length of the empty space at the end of the bitstream buffer is specified by extra.
     *
     * @param len   The buffer accessible length.
     * @param extra The number of bytes to empty at the end of the buffer.
     */
    ...
    /**
     * Huffman decoding of one channel within one granularity group.
     *
     * @param region             region[0..2] is the length of the main data area used by the code table for decoding,
     *                          and region[3] is the total length of the Huffman bits.
     * @param table_select       Decode the index of the large value area code table.
     * @param count1table_select Decode the index of the code table for the small value area (count1 area)
     * @param hv                 Receive the decoded 576 values.
     * @return 576 minus the length of the zero value area.
     */
    public int decodeHuff(int[] region, int[] table_select, int count1table_select, int[] hv) {
        ...
        /*
         * 1. Initialize num, mask, part3len
         * mask: Temporary bitstream buffer does not exceed 32 bits of data, bitstream 2-level buffer
         * num: the number of bits remaining in mask
         * part3len: the number of bits of Huffman-coded main data (main_data)
         */
        ...
        /*
         * 2. Make the bitstream buffer byte-aligned
         */
        if (num > 0) {
            mask = getBits9(num);
            mask <<= 32 - num;
        ...
        for (i = 0; i < 3; i++) {
            ...
            linbits = lin[tmp];
            while (idx < maxidx) {
                if (part3len + num <= 0) { // Check if the bit stream has errors
                    num -= part3len + num;
                    break;
                }
                ...
                x = y >> 8; // x temporary storage hlen
                num -= x;
                mask <<= x;

                x = (y >> 4) & 0xf; // Solve for x,y
                y &= 0xf;
                ...
                if (x != 0) {
                    if (x == 15 && linbits != 0) {
                        ...
                        x += mask >>> (32 - linbits); // Circular right shift
                        num -= linbits;
                        mask <<= linbits;
                    }
                    hv[idx++] = (mask < 0) ? -x : x;
        ...
        while (idx < 572) {
            ...
            // After modifying num, it is immediately checked whether the main data
            // (of the current channel within the current granularity) has been processed,
            // so that files with damaged data can continue to be decoded.
            if (part3len + num <= 0) {
                num -= part3len + num;
                break;
            }

            // A codeword (hcod) is decoded to get 4 values
            if ((y <<= 28) < 0) {
                hv[idx++] = mask < 0 ? -1 : 1;
                num--;
                mask <<= 1;
        ...
        if (num > 0) { // num bits are returned to the bitstream buffer. This can also be done by calling skipBits(-num)
            bitPos = -num; // Step 6 also uses num to temporarily store bitPos
        ...
        /*
         * 5. rzone area is directly set to 0, that is, hv[nonzero..575]=0
         */
        int nonzero = idx;
        while (idx < 576)
            hv[idx++] = 0;
        ...
        /*
         * 6. Discard the ancillary bit. How many bits should the ancillary bit not exceed?
         */
        part3len += num;
        if (part3len > 0) {        // This is not necessarily an auxiliary bit, it may also happen if there is an error in the bit stream
            while (part3len > 9) {
                getBits9(9);    // No longer byte-aligned
                part3len -= 9;
            }
            getBits9(part3len);
        }
    ...
    private void initialize() {
        // Code table for decoding large value areas htbvx[]: 16 arrays with a total of 2752 elements
        short[] htbv0 = {0, 0, 0, 0};
        ...
        // Code table for decoding small value area (count1 area) htcx[]: 2 arrays with 96 elements
        htc0 = new short[] {    //80
