...
public class NesCPU implements IDevice {
    ...
    /**
     * Default initialization. Without setting NES_BASECYCLES, it won't work.
     */
    public NesCPU() {
        this(-1);
    }
    ...
    public int exec(int clock) {
        ...
        return context.clock; // Returns the number of clocks actually running
    }
     ...
     * @param start_adr
     * @param int_adr
     * @param int_freq  Frame rate, default 60
     * @param a         CPU register A Initial value, default value 0
     * @param x         CPU register X Initial value, default value 0
     * @param y         CPU register Y Initial value, default value 0
     */
    public void start(int start_adr, int int_adr, double int_freq, int a, int x, int y) {
        ...
        for (int i = 0; (i < (NES_BASECYCLES / int_freq)) && !broken; i++, context.exec()) {
            pc_count++;
            if (context.pc == breakpoint)
                broken = true;
        }
    ...
    final ReadHandler readByte = adr -> {
        IntHolder val = new IntHolder(0);
        read(adr, val, 0);
        return val.val;
    };

    final WriteHandler writeByte = (adr, value) -> write(adr, value, 0);
