...
/**
 * MMC5 Audio Chip, manages audio output for MMC5 Pulse1, MMC5 Pulse2 channels
 * (MMC5 PCM channel not included)
 *
 * @author Zdream
 * @since v0.2.4
 */
...
//			pulse.looping = (value & 0x20) != 0; // true
//			pulse.envelopeFix = (value & 0x10) != 0; // true
                pulse.fixedVolume = (value & 0xF);
                break;

            case 1:
                // MMC5 doesn't have sweep related control commands, so here is the direct return.
                break;
    ...
    public AbstractNsfSound getSound(byte channelCode) {
        return switch (channelCode) {
            case CHANNEL_MMC5_PULSE1 -> pulse1;
            case CHANNEL_MMC5_PULSE2 -> pulse2;
            default -> null;
        };
    }
