...
//		if (initialized) return;
     ...
     */
    static final int[] logtbl = new int[1 << LOG_BITS];
    ...
    static class FDS_WG {
        ...
        final byte[] wave = new byte[0x40]; // 64 个单位
//		/**
//		 * uint8
//		 */
//		int wavptr;
...
//			wavptr = 0;
    ...
    static class FDS_OP {
        ...
        final FDS_EG eg = new FDS_EG();
        final FDS_PG pg = new FDS_PG();
        final FDS_WG wg = new FDS_WG();
        int bias;
        /**
         * uint8
         */
    ...
    static class FDSSOUND {
         ...
         */
        int lvl;
        final int[] mastervolumel = new int[4];
        /**
         * uint32
         */
        int mastervolume;
    ...
    static final FDSSOUND fdssound = new FDSSOUND();
        ...
        if (fdssound.op[1].wg.disable != 0)
            ...
            while (spd > 0) {
                ...
                if (spd >= left) // advancing to the next entry
                {
                    ...
                    // length: 8
                    byte[] MOD_ADJUST = {0, 1, 2, 4, 0, -4, -2, -1};
                    if (value == 4)
            ...
            int mod = fdssound.op[1].bias * fdssound.op[1].eg.volume;
            mod >>= 4;
            if ((mod & 0x0F) != 0) {
    ...
    static void FDSSoundWrite(int address, int value) {
        if (0x4040 <= address && address <= 0x407F) {
            fdssound.op[0].wg.wave[address - 0x4040] = (byte) ((value & 0x3f) - 0x20);
