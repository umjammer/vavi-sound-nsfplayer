...
import static com.zdream.famitracker.FamitrackerTypes.EF_ARPEGGIO;
...
import static com.zdream.famitracker.FamitrackerTypes.EF_TREMOLO;
import static com.zdream.famitracker.FamitrackerTypes.EF_VIBRATO;
import static com.zdream.famitracker.FamitrackerTypes.EF_VOLUME_SLIDE;
...
public abstract class ChannelHandler extends SequenceHandler {
    ...
    protected void periodAdd(int step) {
        if (m_bLinearPitch)
            linearAdd(step);
        else
    ...
    protected void periodRemove(int step) {
        if (m_bLinearPitch)
            linearRemove(step);
        else
            setPeriod(getPeriod() - step);
    ...
    protected void linearAdd(int step) {
    ...
    protected void linearRemove(int step) {
        m_iPeriod = (m_iPeriod << 5) | m_iPeriodPart;
        int value = (m_iPeriod * step) / 512;
        if (value == 0)
            value = 1;
     ...
     */
    private final int m_iMaxPeriod;

    /**
     * Max channel volume
     */
    private final int m_iMaxVolume;
